...
function setItemToEdit(li) {
  ...
  // Change button to "Update Item" and set color to green
...
// Reset the button back to "Add Item" after update
...
function checkUI() {
  ...
  // Reset the button back to default state if not in edit mode
  if (!currentEditItem) {
    resetEditMode();
  }
}
