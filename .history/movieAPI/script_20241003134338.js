...
function updateItem(newText) {
  if (currentEditItem) {
    // Update the text of the current list item (ignoring the buttons)
    currentEditItem.firstChild.nodeValue = newText;

    // Update local storage
    let itemsFromStorage = getItemFromStorage();

    // Extract the old text from the list item, ignoring the "Update" and "Remove" buttons
    const oldText = currentEditItem.firstChild.nodeValue.trim();

    // Find the index of the old text in local storage
    const index = itemsFromStorage.indexOf(oldText);

    if (index > -1) {
      // Replace the old text with the new text in the local storage array
      itemsFromStorage[index] = newText;
...
function checkUI() {
  ...
  updateButton.style.backgroundColor = "#0056b3";
}
