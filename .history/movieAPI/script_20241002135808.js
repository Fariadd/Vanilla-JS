...
function removeItemFromStorage(event) {
  if (event.target.classList.contains("remove-btn")) {
    const li = event.target.parentElement; // Get the <li> element
    const itemText = li.textContent.replace("Remove", "").trim(); // Extract the text without the "Remove" button text

    // Remove the item from the UI
    li.remove();

    // Remove the item from localStorage
    let itemsFromStorage = getItemFromStorage(); // Retrieve items from storage
    itemsFromStorage = itemsFromStorage.filter((item) => item !== itemText); // Filter out the removed item
    localStorage.setItem("item", JSON.stringify(itemsFromStorage)); // Update localStorage
  }
}
