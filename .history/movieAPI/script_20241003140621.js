...
function removeItemFromStorage(event) {
  // Check if the clicked target is a "Remove" button
  if (event.target.classList.contains("remove-btn")) {
    if (confirm("Delete this item?")) {
      const li = event.target.parentElement; // Get the parent <li> element
      const textItem = li.firstChild.textContent.trim(); // Get only the text content (trim any spaces)
      ...
      // Filter out the item from the storage array
  ...
  checkUI(); // Update UI (show/hide buttons or filters)
}
